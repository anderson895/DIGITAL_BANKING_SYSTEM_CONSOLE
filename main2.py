...
bank_data = {
    ...
    "system_balance": {1000: 1, 500: 1, 200: 3, 100: 0, 50: 0, 20:0, 10:0, 5:0, 1:0}
}
...
def can_withdraw(amount):
    # Check if a valid combination exists and return the used bills if so
    return find_combination(amount, bank_data["system_balance"].copy()) is not None



def find_combination(amount, available_bills):
    if amount == 0:
        return {}  # Success, return an empty dict to track used bills

    for denomination in denominations:
        if available_bills[denomination] > 0 and amount >= denomination:
            # Take one bill of this denomination
            available_bills[denomination] -= 1
            result = find_combination(amount - denomination, available_bills)
            if result is not None:
                # If the result is valid, add this denomination to the result
                if denomination in result:
                    result[denomination] += 1
                else:
                    result[denomination] = 1
                return result
            available_bills[denomination] += 1  # Backtrack if not successful

    return None  # If no combination is found


def dispense_cash(amount):
    result = find_combination(amount, bank_data["system_balance"])
    if result:
        print("Dispensing:")
        for denomination, count in result.items():
            print(f"{count} x PHP {denomination}")
            bank_data["system_balance"][denomination] -= count
    else:
        print("Error: Unable to dispense cash.")
